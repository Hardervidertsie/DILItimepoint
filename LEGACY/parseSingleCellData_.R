...
## Functions
	...
	.parseOutputlist <- function(outputList, onlyPipettingIssues = FALSE, .debug = FALSE, TimesAboveDMSO = 2, ...) {
		...
		for(rep in names(parsedSDs)) {
			...
			DMSOstats <- do.call(rbind, lapply(plateIDs, function(.pID) {
			...
			parsedSDs[[rep]]$fractionAnV <- sapply(parsedSDs[[rep]]$matchDoseTime, function(x) {
				AnV <- parsedDTs[[rep]]$PI_masked_primaryID_AreaShape_Area[which(parsedDTs[[rep]]$matchDoseTime == x)]
				sum(!(is.na(AnV)))/length(AnV)
			} )
		}

		# add number of objects per well
		for(rep in names(parsedSDs)) {
	...
	plotDR <- function(reporter, compound) {
		plotData <- melt(do.call(rbind, lapply(reporter$summaryDatas, function(x) as.data.frame(x[x$treatment == compound, ])))[, c('treatment', 'timeID', 'CMAX_numeric', 'fractionAboveMean')], 
		...
		P <- ggplot(data = plotData, aes(x = CMAX_numeric, y = value, group = timeID, colour = timeID)) +
				...
				ylab('Fraction >2x DMSO') + xlab('CMAX') + labs(title = compound) + ylim(c(0,1))
